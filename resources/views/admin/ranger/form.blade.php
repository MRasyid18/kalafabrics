...
@section('title', isset($user) ? 'Edit Ranger' : 'Tambah Ranger')
@section('page-title', isset($user) ? 'Edit Ranger' : 'Tambah Ranger')

@section('content')
<div class="page-header">
    <h1>{{ isset($user) ? 'Edit Data Ranger' : 'Tambah Ranger Baru' }}</h1>
    <p>Silakan isi informasi akun di bawah ini.</p>
</div>

<div class="card" style="max-width:600px;">
    <div class="card-body" style="padding:24px;">
        <form action="{{ isset($user) ? route('admin.rangers.update', $user->id) : route('admin.rangers.store') }}" method="POST">
            @csrf
            @if(isset($user)) @method('PUT') @endif
            
            <div style="margin-bottom:16px;">
                <label style="display:block; font-size:13px; font-weight:600; margin-bottom:6px; color:#6b6b5a;">Nama Lengkap</label>
            ...
            <div style="display:flex; gap:12px;">
                ...
                <a href="{{ route('admin.rangers.index') }}" style="background:#f0ede6; color:#1e2318; padding:12px 24px; border-radius:8px; font-weight:600;">Batal</a>
            </div>
        </form>
    </div>
</div>
